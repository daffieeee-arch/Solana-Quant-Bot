...
6. After merging, verify the resulting `main` commit and technical/security main checks. Report technical delivery and administrative Project synchronization separately. Technical, security or evidence failures remain blockers. A demonstrated external Projects availability/visibility failure does not block otherwise safe delivery or further development; preserve it as an open administrative point, never a successful sync. Follow the cadence and backlog rule below.
...
Report the PR, review findings and resolutions, reviewed commit, tests, squash-merge commit, separate technical-main and administrative-sync results, and exactly what was cleaned up or deliberately preserved.

Owner amendment, 2026-09-23: Roadmap Sync runs only on main pushes, the existing daily schedule, and deliberate workflow_dispatch on main. Keep issue/PR content current; the board follows main updates and normally no later than the next daily sync. After more than 24 hours without a successful sync, report the backlog and last successful run; do not start automatic diagnosis/retry loops. An important status change without a main update may receive one manual trusted-main sync after changes are batched. Preserve trusted-main code/config, token restrictions, serial non-cancelling execution and all fail-closed synchronization checks; never bypass branch protection. See docs/operations/GITHUB_PROJECTS_ROADMAP.md.
