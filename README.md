...
- Immutable functional baseline quality: 592 tests. Phase 4 was squash-merged through PR #5 as a transport-free Old Faithful/Jetstreamer contract boundary. The fixture-verified Phase-5 Rust reducer was squash-merged through PR #7 as `9739eed415c90e4433b77e0cabc46bd32577bb9e`. Phase 6A was squash-merged through PR #9 as `95511866c31b6325c2341b5b7a3b98c9503d85d1`; post-merge main CI run `32231262382` passed Node 953/953 across 70 files, Rust 60/60 under Rust 1.97.1, TypeScript, backend/frontend build, repository policy, transport isolation, push integrity, and clean-tree gates. Phase 6A is synthetic and fixture-only, has processed no real archive/CAR/slot data, provides no OOS or profitability evidence, and remains explicitly `researchReady: false`.
- The Phase 6B feature branch adds only an offline synthetic exact-byte Pump curve, mint, base-token-account and v2 quote-token-account state/provenance contract with shared TypeScript/Rust vectors. It is not wired to Jetstreamer callbacks or reducer execution, claims no real activation range, and keeps `approved: false`, `researchReady: false`, and `pilotEligible: false`; see [`docs/PHASE6B_PUMP_SILVER_STATE_PROVENANCE_CONTRACT.md`](docs/PHASE6B_PUMP_SILVER_STATE_PROVENANCE_CONTRACT.md).
