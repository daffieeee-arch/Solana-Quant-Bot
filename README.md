...
- Immutable functional baseline quality: 592 tests. Phase 4 was squash-merged through PR #5 as a transport-free Old Faithful/Jetstreamer contract boundary; post-merge main CI passes 785 tests across 68 files under Node 22 plus TypeScript, build, repository-policy, and transport-isolation gates. The fixture-verified Phase-5 Rust reducer is committed and pushed in open PR #7, remains unmerged, has processed no real archive/CAR/slot data, and remains explicitly `researchReady: false`.
...
ClickHouse contains the paused v1 `TRANSACTION_NET_SWAP` dataset. It supports transaction-level net-flow forensics, not a complete event-level tape or Pump strategy evidence. Bronze capture and the merged Phase-4 archive contract boundary exist. The read-only Jetstreamer Rust reducer is fixture-verified, committed, and pushed in open PR #7, but remains unmerged and has not processed real archive/CAR/slot bytes; `researchReady` remains `false`, and Silver/Gold event/state layers remain required.
