...
- Phase 8D stopped safely before mutation because Hermes has no container daemon/socket/builder. Phase 8D1 is an uncommitted `REMOTE_ISOLATED_GITHUB_BUILDER` candidate: no-push PR verification plus a separate future manual private-GHCR publish design. Remote image/container tests are not executed until later authorized delivery; see [`docs/PHASE8D1_REMOTE_IMAGE_BUILD_GHCR_READINESS.md`](docs/PHASE8D1_REMOTE_IMAGE_BUILD_GHCR_READINESS.md).
