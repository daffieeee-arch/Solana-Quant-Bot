...
- A later **Pilot A** may test only Old Faithful event/transport evidence after a separately reviewed Pilot A Readiness Package, bandwidth-cap preflight and explicit execution GO. It may prove blocks, transactions, instructions, logs, events, coverage, quarantine and deterministic reruns without first requiring a historical raw-account-state source, but it may never claim raw state, liquidity, position-size impact, execution-grade returns, OOS readiness or profitability.
- **Pilot B** remains NO-GO until a reliable raw-account-state source, causal binding and real activation/layout boundaries are proven. The next step is only to design and exactly freeze the Pilot A Readiness Package—not to run Pilot A, download a CAR or stream an archive. The separate Old Faithful thread remains paused and read-only.
