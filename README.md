...
- The offline Phase-7 Pilot A readiness candidate is implemented and documented in [`docs/PHASE7_OLD_FAITHFUL_PILOT_A_READINESS.md`](docs/PHASE7_OLD_FAITHFUL_PILOT_A_READINESS.md). Official epoch-978 metadata and the 1,000-slot range are pinned, but every registry entry is `STRUCTURALLY_SUPPORTED_UNPROVEN_ACTIVATION`; verdict `HOLD_UNPROVEN_ACTIVATION`, `approved: false`, `researchReady: false`, and `pilotEligible: false`.
- A later **Pilot A** remains blocked pending exact-byte review, a separately authorized bandwidth-cap preflight, proven slot activation, and explicit execution GO. **Pilot B** remains NO-GO until reliable raw account state and causal binding are proven. No CAR/range download, archive stream, slot processing, ClickHouse/backfill action, OOS run, or profitability claim is authorized.
