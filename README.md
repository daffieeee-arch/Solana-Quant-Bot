...
- PR #18 squash-merged Phase 8A as `404019d3562afd3c10c1f65da141ab4e3f5ba1fc`; post-merge CI run `32466795771` passed Node 1,273/1,273 across 78 files, Rust 83/83 including the focused runner 15/15, TypeScript/build/transport/policy/integrity gates, and clean-tree proof. It remains `SYNTHETIC_FIXTURE_ONLY`, `HOLD_UNPROVEN_ACTIVATION`, `researchReady: false`, `strategyInputEligible: false`, and `pilotEligible: false`.
- PR #19 squash-merged Phase 8C as `6c597b4c2c274583a4fdd9ba7a24db44c21df5a8`; post-merge CI run `32501444570` passed Node 1,341/1,341, Rust 83/83 and the cockpit/image-policy gates. No image, dataset, app or Grafana object was deployed.
- PR #21 merged Phase 8D1 publish hardening as `9ed8d5b8d8b67284c8fc20c164f6816bbfc0c180`. Authorized run `32641496527` pushed two immutable GHCR versions and then stopped at `BOTH_PUSHED_RETEST_REQUIRED_HOLD`; `deploymentEligible` remains false. Phase 8D1-R is a not-yet-dispatched read-only recovery candidate for those exact digests. No pull credential, dataset, app or deployment exists; see [`docs/PHASE8D1_REMOTE_IMAGE_BUILD_GHCR_READINESS.md`](docs/PHASE8D1_REMOTE_IMAGE_BUILD_GHCR_READINESS.md) and [`docs/PHASE8D1_EXISTING_DIGEST_RECOVERY.md`](docs/PHASE8D1_EXISTING_DIGEST_RECOVERY.md).
