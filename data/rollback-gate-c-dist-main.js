...
import { MarketContextProvider } from './providers/market-context.js';
...
async function loadRecentDecisions(dataDir) {
    try {
        ...
        for (const line of history.trim().split('\n')) {
            ...
            for (const rawDecision of record.decisions) {
                ...
                if (typeof decision.type === 'string' && typeof decision.pairId === 'string')
                    decisions.push({ ...decision, at: typeof record.at === 'string' ? record.at : undefined });
            }
        }
...
async function loadDuplicateSuppressed(dataDir) { try {
    const history = await readFile(`${dataDir}/events.ndjson`, 'utf8');
    let count = 0;
    for (const line of history.trim().split('\n')) {
        if (!line)
            continue;
        const event = JSON.parse(line);
        count += event.decisions?.filter((decision) => decision.type === 'duplicate_suppressed').length ?? 0;
    }
    return count;
}
catch (error) {
    if (error.code === 'ENOENT')
        return 0;
    console.error(JSON.stringify({ event: 'dashboard_duplicate_history_unavailable', mode: 'paper', message: error instanceof Error ? error.message : String(error) }));
    return 0;
} }
async function loadClosedTrades(dataDir) {
    try {
        const history = await readFile(`${dataDir}/events.ndjson`, 'utf8');
        const trades = [];
        for (const line of history.trim().split('\n')) {
            ...
            for (const rawDecision of record.decisions) {
                if (!rawDecision || typeof rawDecision !== 'object')
                    continue;
                const decision = rawDecision;
                if (decision.type === 'paper_exit' && typeof decision.pairId === 'string')
                    trades.push({ ...decision, at: typeof record.at === 'string' ? record.at : undefined });
            }
        }
...
async function loadEquityHistory(dataDir) {
    try {
        const history = await readFile(`${dataDir}/events.ndjson`, 'utf8');
        const points = [];
        for (const line of history.trim().split('\n')) {
            ...
            if (!event || typeof event !== 'object')
                continue;
            const record = event;
            if (typeof record.at === 'string' && typeof record.equityLamports === 'number' && Number.isSafeInteger(record.equityLamports))
                points.push({ at: record.at, equityLamports: record.equityLamports });
        }
