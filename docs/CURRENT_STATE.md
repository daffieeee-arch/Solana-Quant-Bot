...
- Phase 8D stopped before mutation because Hermes has no container builder/runtime. PR #20 squash-merged Phase 8D1 remote verification as `8b5ecb6168ac3d1ea9fa6630ab8a43ada1b686e8`; post-merge CI `32586030668` and main verify-only run `32586405638` succeeded. The private GHCR publish workflow remains never dispatched, no package was pushed, and every TrueNAS/Grafana/ClickHouse/HOLD nonclaim remains unchanged. This follow-up only hardens the unexecuted publish path with an exact BuildKit server pin, job-local Node/policy bootstrap and durable partial-publish HOLD state.
