...
- Current research branch: `phase3/pump-v2-bronze-pilot`
...
- PR #1 adds adversarial CI-policy coverage; Phase 2 adds the Pump historical-research harness; Phase 3 adds Bronze capture, transitive transport-policy plus syscall isolation, forensic-generator, and immutable-manifest contract tests. The fresh complete Node-22 suite passes 742 tests across 67 files after the latest NO-GO remediation.
...
5. The Bronze capture contract resolves complete static + loaded writable + loaded readonly account vectors, but the real Old Faithful/Jetstreamer adapter that supplies those fields remains unimplemented and unproven.
...
Independently review and land the transport-free Phase 3 Bronze capture contract. Then build a separate read-only Old Faithful/Jetstreamer adapter and run only a tiny approved fixture/slot pilot before one complete epoch. Do not start ClickHouse/backfill, tune on v1, or claim OOS readiness from Bronze capture alone.
