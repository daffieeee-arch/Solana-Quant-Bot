...
- PR #19 squash-merged the GO/GO-reviewed Phase-8C cockpit-only runtime and Grafana architecture as `6c597b4c2c274583a4fdd9ba7a24db44c21df5a8`; post-merge run `32501444570` passed Node 1,341/1,341, Rust 83/83 and all cockpit/image-policy gates. Contracts remain unapplied.
- PR #21 merged Phase 8D1 publish hardening as `9ed8d5b8d8b67284c8fc20c164f6816bbfc0c180`; post-merge CI `32632512662` and verify-only run `32632867961` passed. Authorized publishrun `32641496527` then pushed both immutable private-GHCR candidates but stopped fail-closed at `BOTH_PUSHED_RETEST_REQUIRED_HOLD`: package metadata evidence was not produced before digest replay, so `deploymentEligible` remains false. Phase 8D1-R added only a not-dispatched read-only existing-digest recovery candidate; its [retired document is preserved at the immutable pre-cleanup tag](https://github.com/daffieeee-arch/solana-paper-scanner/blob/v1-paper-platform-final/docs/PHASE8D1_EXISTING_DIGEST_RECOVERY.md). No TrueNAS/Grafana/ClickHouse mutation or pull credential exists.
- Phase 8D1 runtime identities remain `SELECTED_READ_ONLY_NOT_APPLIED`: runner `61000`, cockpit `61001`, shared GID `61000`. Two complete canonical read-only audits match at 621 records / 87 dataset mount owners; legacy 55/59 counts were incomplete transport-success subsets, not ownership changes. Any selected-ID collision or unexplained drift remains blocking, and Phase 8D2 must recheck live before applying anything.
