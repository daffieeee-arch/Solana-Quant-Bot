...
PR #2 and the PR #3 CI-race hotfix are merged with green post-merge CI. Current work is isolated on `phase3/pump-v2-bronze-pilot`. Subsequent independent exact-hash reviews exposed adversarial CPU/sparse-array/transport, forensic-identity/part-classification, ancestor-symlink, computed-capability/HTTP2, lexical-scope shadowing, native-addon forwarding, parser Git-state/index drift, orphan payload/mark stems, and stack-height defects. The newest remediation adds runtime all-deny process loaders plus seccomp network-syscall isolation, lexical-symbol constant resolution, an exercised exact `fs-ext` exception, strict equality of payload/mark stem sets, optional-lock-free Git reads, and end-to-end provenance validation. These fixes are local and the fresh complete Node-22 suite passes 742 tests across 67 files; a new staged hash, final independent review, and commit binding remain required.
