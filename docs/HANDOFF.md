...
Phase 5 is merged and remains fixture-only with `researchReady: false`. Stop after the docs-sync PR and begin any next development phase in a new session. Separate approval before any bounded real slot pilot or CAR download remains mandatory. Silver events/state, causal rug-risk parity, registry approval, exact `[T0,T3)` backfill, and walk-forward research remain later gates.
