...
- **Recorded research-pilot processing stop:** the subsequently acquired fixed `[422669516,422669519)` selection retains its native `RESEARCH_SAMPLING` identity. Its [offline B5 attempt](research/B5_COVERAGE_AND_PILOT.md#recorded-pilot-offline-attempt-2026-09-14) verifies 3,224 archival Transaction envelopes but stops at the unchanged 16 MiB per-slot serialized-record cap. No complete pilot Bronze/Silver, Parquet dataset or DuckDB result is published. This is an engineering resource stop, not insufficient-data or edge-falsification evidence; Research Ready remains false and no delivery is promoted.
