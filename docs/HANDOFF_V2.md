...
- **B4 offline remainder (not live B4B):** The OF1 memo, remainder contract, candidate pin review and offline implementation are [`research/OF1_QUANT_RESEARCH_MEMO_V2.md`](research/OF1_QUANT_RESEARCH_MEMO_V2.md), [`research/B4_OFFLINE_REMAINDER.md`](research/B4_OFFLINE_REMAINDER.md), [`research/JETSTREAMER_V0_7_0_PIN_REVIEW.md`](research/JETSTREAMER_V0_7_0_PIN_REVIEW.md) and `scripts/b4-offline-remainder.mjs`. They do not authorize a download or complete B4. The Jetstreamer SHA is a candidate pin only. Live B4B stays reserved for a later separately approved lease.
...
The [Rust index planner](research/OF1_RUST_PLANNER.md) provides fixture-only index-to-byte planning. The second repair adds a [fixture Raw/receipt store](research/OF1_DURABLE_RAW_STORE.md) and [executed durability report](research/OF1_DURABILITY_EVIDENCE.md): immutable pair publication, durable attempts and restart-surviving deadlines. These are not acquisition evidence. The JS paths remain provisional until the separate integration/parity replacement; no live lease is implied.
...
| PR 4 / B4 | **In progress, no acquisition run authorized:** bounded authentic Old Faithful acquisition with live terminal/TUI progress. PR #94 B4A is a `Fixture` partial result only. Next honest package is the offline remainder in [`research/B4_OFFLINE_REMAINDER.md`](research/B4_OFFLINE_REMAINDER.md), not live B4B |
