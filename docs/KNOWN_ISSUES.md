...
1. **Project #4 is not rebased yet.** Current fields/views, Project description/README, Evidence options and issue metadata still express the paper-first roadmap. The current sync does not write `V2 Phase`/`V2 Disposition`, scans the complete PR history and unconditionally reactivates archived repository items. After PR 1, first create the dedicated governance issue and merge the bounded tested sync/config extension in [`PROJECT_V2_REBASE.md`](PROJECT_V2_REBASE.md); only then create successors or V2 metadata. No PR 2A starts until reconciliation, retention and closure evidence pass.
