...
25. Pilot A remains `HOLD_UNPROVEN_ACTIVATION` pending every separate readiness and authorization gate. The next development milestone is an eligibility split, Bronze-only runner, and Research Cockpit/observability adapter, none implemented here. Pilot B, accepted Silver, full epoch, multi-month research, OOS, execution-quality, and profitability claims remain NO-GO.
