...
25. **The pinned toolchain is host-local, not an implicit installer contract.** On 2026-09-02 the approved WSL host gained side-by-side Node `22.23.2`, Rust/Cargo `1.97.1`, rustfmt/clippy and the minimal native build prerequisites, exposed through an untracked user-owned wrapper. Future agents must still run the doctor first and may not install, update profiles or assume another host has parity without explicit approval.
