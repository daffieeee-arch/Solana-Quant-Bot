...
29. **Hermes has no safe local container builder:** Phase 8D stopped before mutation rather than mounting Docker/containerd sockets or weakening isolation. Phase 8D1 remote verification is merged and green on a GitHub-hosted runner. The private publish route remains never dispatched and is separately gated; this follow-up pins the actual BuildKit server and adds durable partial-publish HOLD/recovery state.
