...
1. **Project #4 is not rebased yet.** The live fields/views, Project description/README, Evidence options and issue metadata still express the paper-first roadmap. PR #69 and the hashed read-only export are complete; issue #70 owns bounded PR #71. `closed_item_retention_days: 30` is approved, and squash merge plus automatic trusted-main reconciliation is explicitly authorized after the documentation correction and full CI pass. Issue #63 was explicitly reopened as a governance correction and current-schema Roadmap Sync run `33555734237` passed; its earlier close was temporally associated with PR #69's merge, but the exact initiating mechanism remains unproven. The immutable snapshot correctly retains the captured closed state, while the reopen is a documented post-snapshot delta. Issue #63 remains open and pinned through initial G0/E0. Only after the reconciliation audit may successors or V2 metadata be created; no PR 2A starts until the full governance evidence passes.
