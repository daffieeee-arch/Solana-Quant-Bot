...
6. **Jetstreamer/OF1 pin unresolved.** [`research/JETSTREAMER_V0_7_0_PIN_REVIEW.md`](research/JETSTREAMER_V0_7_0_PIN_REVIEW.md) accepts `cffaf3d891b3cbe45a46dd963d6d3571b2aa1a24` as the initial V2 acquisition crate *candidate* pin only. It is not a blessed live acquisition crate. Exact live index/sidecar identities, host/redirect allowlisting and a leased path remain open. PR #94 B4A is an offline fixture and does not pin a live Jetstreamer/OF1 client.
7. **Raw capture and resume correctness remain open.** The JS remainder can overwrite evidence and accept new bytes with an old/minimal receipt. The earlier PR94 recorder can lose failed request attempts and reset runtime on restart. The [Rust planner](research/OF1_RUST_PLANNER.md) fixes only the index/range-planning boundary; durable publication and budget/resume fixes are subsequent bounded PRs. No live capture path or complete offline recorder is proven. Never treat the existing fixture helpers as acquisition-ready.
8. **No acquisition run is approved.** No Triton/OF1 call, credit spend or full epoch is authorized. B4A exists only as an offline fixture against injected bytes. [`research/B4_ENGINEERING_VALIDATION_LEASE_PLAN.json`](research/B4_ENGINEERING_VALIDATION_LEASE_PLAN.json) is a filled draft lease with `approved: false` and `networkEnabled: false`; writing it is not approval. Every host/range/request/byte/disk/runtime/concurrency value remains a preregistered per-run parameter until those two flags and operator/`approved_at`/`cost_confirmation` are set by a human.
