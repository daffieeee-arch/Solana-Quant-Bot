...
7. **Raw-byte capture hook unproven for live acquisition.** V2 must retain exact acquired ranges/content blocks and receipts before parsing. The offline remainder now persists raw bytes, identity and a receipt before parse against injected fixtures (`scripts/b4-offline-remainder.mjs`). That does not prove a live Jetstreamer/OF1 capture hook. If the later leased path cannot expose raw bytes, resolve the adapter before calling output Raw.
