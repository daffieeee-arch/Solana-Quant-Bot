...
The static transport policy retains generic descriptor reflection as forbidden. Its only exception is the exact built-root `pump-silver-contract.js` module, the direct top-level `normalizeFixtureRegistryEntry` function declaration with exactly one parameter named `entry` and that exact parameter symbol as the descriptor argument, and the four direct syntactic string literals `startInclusive`, `endExclusive`, `officialDocsCommit`, and `pumpIdlSha256`. Computed, concatenated, aliased, shadowed, nested-scope, wrong-file, wrong-function, wrong-parameter, or unknown-key descriptor reads remain forbidden.
