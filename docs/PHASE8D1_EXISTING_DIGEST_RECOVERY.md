...
Package metadata uses at most six attempts with at most 15 seconds between attempts and no more than 90 seconds total, including bounded requests and complete same-origin `Link: rel=next` version pagination. HTTP 401/403 stops immediately. A persistent 404, timeout, incomplete/cross-origin pagination, public package, missing repository link, tag drift, public unauthenticated pull, missing attestation or any digest-retest failure remains `RECOVERY_HOLD`.

## Recovery evidence

The recovery downloads and verifies the original artifact chain without replacing it. Artifact archives use two isolated hops: the authenticated GitHub API request negotiates `application/vnd.github+json` and accepts only a manual `302 Location`; the validated absolute HTTPS signed URL is then fetched without authorization, cookies, or GitHub API headers. ZIP type/path/declared-size validation completes before bounded regular-file extraction. It checks unauthenticated denial plus authenticated read; reads OCI index, linux/amd64 manifest/config, subject-bound readable SLSA provenance and SPDX SBOM; pulls only the two fixed registry digests; and runs the original source checkout's complete runner/cockpit verification. Complete package-version/tag inventories are persisted before and after. After the long retest, package visibility, repository link, unauthenticated denial and both live tag digests are checked again. Base drift, both Git trees and every package-version/tag byte must remain unchanged.
