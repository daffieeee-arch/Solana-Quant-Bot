...
Phase 8C adds only a separately built cockpit-only runtime, deployment/image/dataset designs and Grafana/ClickHouse architecture. The old dashboard/data path is `LEGACY_FORENSIC_V1 / FORENSIC_ONLY`; new research domains remain design-only. Phase 8C does not change `SYNTHETIC_FIXTURE_ONLY`, `HOLD_UNPROVEN_ACTIVATION`, `researchReady: false`, strategy eligibility, accepted Silver or Pilot A/B authorization. See [`PHASE8C_COCKPIT_ONLY_RUNTIME_GRAFANA_ARCHITECTURE.md`](PHASE8C_COCKPIT_ONLY_RUNTIME_GRAFANA_ARCHITECTURE.md).
