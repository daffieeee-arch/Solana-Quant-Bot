...
Pool-depth snapshots are currently rejected, not simulated. The shared production 6-decimal-base/9-decimal-quote conversion requires a separate correction and independent review before historical fill-impact claims are allowed.
...
`npm run build` also executes `verify:research-transport` against the actual compiled CLI under ESM and CommonJS import guards. Loading HTTP(S), net/TLS, WebSocket/fetch transports, or the broad `@solana/web3.js` root fails the build. Pump PDA derivation uses a narrow local crypto/base58 primitive whose bytes are regression-tested against `PublicKey.findProgramAddressSync`.

## Next data step

Do not tune strategy parameters on v1. The next data task is a separate v2 Old Faithful parser/export pipeline that records native SOL deltas, inner instructions, loaded addresses, canonical Pump discriminator+PDA identity, instruction/event ordering, canonical launches, causal feature windows, scan cycles/cadence, and the remaining live-gate snapshot fields. Its parser/query tuple must be independently reviewed and added to the registry before any train/validation/test result is considered evidence.
