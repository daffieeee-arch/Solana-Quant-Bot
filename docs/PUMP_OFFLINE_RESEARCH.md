...
Do not tune strategy parameters on v1. `docs/PHASE3_PUMP_V2_PILOT.md` defines the first Bronze capture vertical slice and `docs/research/V1_FORENSIC_MANIFEST.json` preserves the read-only v1 filesystem evidence with writer state explicitly unverified by the generator. Bronze capture alone is not a dataset and cannot enter the provenance registry. The local Phase-4 checkpoint now implements only the reviewed transport-free Old Faithful source-manifest, Jetstreamer envelope, persisted-observation validation, and slot-coverage contract boundary. The next data task after review and a separate explicit GO is a real read-only Rust reducer and durable file writer against the pinned Jetstreamer callback types; CAR verification, Silver event/state decoding, causal features, scan cadence, live-gate snapshots, registry approval, and chronological research remain later gates.
