...
Phase 7A activation research is complete. PR #16 merged the evidence and tracked offline citationgate as `784192a675e31d78da82852e91ceb254eccae982`; post-merge run `32397224604` is green with the separate citation step active. The result remains 0/10 proven because historical ELF/source mapping and raw candidate-range observations are absent.

Phase 8A adds only a synthetic fixture eligibility split, a direct Rust Bronze runner around the existing Phase-5 reducer, a bounded read-only provider, a lazy Research Cockpit and pure observability/Grafana contracts. Runner output is `SHADOW_STRUCTURAL_OBSERVATION`, `UNPROVEN_ACTIVATION`, `NOT_ACCEPTED_SILVER` and `NOT_STRATEGY_INPUT`. It may never feed strategy scoring, paper entries/exits, OOS or profitability analysis. **Pilot B** remains NO-GO; no accepted Silver, preflight, payload retrieval, shaping or pilot is authorized.

Phase 8C added only a separately built cockpit-only runtime, deployment/image/dataset designs and Grafana/ClickHouse architecture. The old dashboard/data path is `LEGACY_FORENSIC_V1 / FORENSIC_ONLY`; new research domains remain design-only. Phase 8C did not change `SYNTHETIC_FIXTURE_ONLY`, `HOLD_UNPROVEN_ACTIVATION`, `researchReady: false`, strategy eligibility, accepted Silver or Pilot A/B authorization. Its [retired architecture document is preserved at the immutable pre-cleanup tag](https://github.com/daffieeee-arch/solana-paper-scanner/blob/v1-paper-platform-final/docs/PHASE8C_COCKPIT_ONLY_RUNTIME_GRAFANA_ARCHITECTURE.md).
