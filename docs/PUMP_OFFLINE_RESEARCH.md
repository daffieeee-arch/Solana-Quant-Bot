...
Do not tune strategy parameters on v1. `docs/PHASE3_PUMP_V2_PILOT.md` defines the first Bronze capture vertical slice and `docs/research/V1_FORENSIC_MANIFEST.json` preserves the read-only v1 filesystem evidence with writer state explicitly unverified by the generator. Bronze capture alone is not a dataset and cannot enter the provenance registry. The next data task after review is a separate read-only Old Faithful/Jetstreamer adapter with immutable source manifests and a coverage ledger; Silver event/state decoding, causal features, scan cadence, live-gate snapshots, registry approval, and chronological research remain later gates.
