...
> **Document status: ACTIVE.** [GitHub Project #4](https://github.com/users/daffieeee-arch/projects/4) is the central delivery roadmap. The owner accepted the [bounded B4/B5 engineering contracts on 2026-09-23](research/B4_B5_ENGINEERING_ACCEPTANCE_20260923.md). B4/#83 and B5/#84 are complete at `Engineering Validation`; B6/#85 is the current `ACTIVE NOW` item with partial mint-inspector, chronological presentation replay, event-reported reserve/address views, descriptive volume/flow/user summaries and three-slot pilot-quality increments; #85 remains open / Unproven. Research Ready remains false. The completed migration ledger and historical source/lease evidence are preserved; no further acquisition is authorized.
...
| PR 4 / B4 #83 | **Accepted / Done / SUPERSEDED / Engineering Validation:** bounded acquisition contract | Authentic plans, budgets, integrity/coverage and clean same-plan resume; separately labelled crash/retry fixtures; [dated acceptance](research/B4_B5_ENGINEERING_ACCEPTANCE_20260923.md) | no new acquisition, authentic crash claim, full epoch/root membership or Research Ready |
| PR 5 / B5 #84 | **Accepted / Done / SUPERSEDED / Engineering Validation:** delivered native Raw → Bronze → Silver | [#132 walking skeleton](research/B5_RAW_BRONZE_SILVER_WALKING_SKELETON.md), [#133 native order parity](research/B5_NATIVE_ORDER_PARITY.md), and [accepted lifecycle interpretation](research/B4_B5_ENGINEERING_ACCEPTANCE_20260923.md): 15 packages / five facts / 58 balances with explicit phase gaps | no new admission, complete lifetime, historical activation, CPI-rights proof, Gold or Research Ready |
| PR 6 / B6 #85 | **In Progress / ACTIVE NOW / Unproven; partial increment:** interactive Research Observatory MVP | [Read-only mint-package inspector, presentation timeline/trade table, event reserve/address and descriptive flow summaries, and bounded pilot-quality view](research/B6_LOCAL_MINT_INSPECTOR.md); remaining #85 requirements require separate scope and evidence | no fake panels, frozen-legacy extension, paper controls or full workbench |
...
The content migration and B2A/B3 closeouts remain historical evidence. B4/#83 and B5/#84 are technically accepted on 2026-09-23 as `Done` / `SUPERSEDED` / `Engineering Validation` within their bounded contracts. B6/#85 is `In Progress` / `ACTIVE NOW` / `Unproven`: its separately approved first read-only mint inspector is a partial increment, not complete Observatory acceptance. B7/#86 is queued `Backlog` / `NEXT` / `Unproven`; this routing does not start B7, and B8 remains `LATER`. Research Ready remains false; no acquisition is authorized. The completed milestones use the existing `SUPERSEDED` routing convention, while their issue close reason is `completed`. The later explicit B6 authorization makes #85 the sole concrete `ACTIVE NOW` delivery. B7/#86 is the queued `NEXT`, not started; B8 remains `LATER`. Disposition never authorizes execution. Original criteria, receipts and slice classes are unchanged; see the dated acceptance decision for the #83-before-#84 closeout procedure.
