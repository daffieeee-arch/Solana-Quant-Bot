...
| PR 4 / B4 #83 | **In progress / ACTIVE NOW / Unproven:** four authentic metadata publications, no CAR payload. The [staged path](research/OF1_STAGED_ACQUISITION.md) retains the original [fixture report](research/OF1_ACQUISITION_EVIDENCE.md) separately | live progress, verified receipts, deterministic resume and explicit coverage/gaps; checksum correction first, graphical download monitor in a separate PR | new [metadata GO](research/OF1_METADATA_RUN_PROPOSAL.md) and separate payload GO required after final binary preparation; no B4 completion, root membership, full epoch, backend override or research claim |
