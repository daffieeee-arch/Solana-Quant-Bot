...
| PR 4 / B4 #83 | **In progress / ACTIVE NOW:** bounded Old Faithful acquisition. The [staged fixed-host path](research/OF1_STAGED_ACQUISITION.md) and [visible TLS/restart/integrity report](research/OF1_ACQUISITION_EVIDENCE.md) are `Fixture` partial results, alongside retained earlier reports | live terminal/TUI progress, verified receipts, deterministic resume and explicit coverage/gaps | [metadata GO](research/OF1_METADATA_RUN_PROPOSAL.md) then separate payload GO still required; no run authorized or B4 completion; root membership unavailable; no full epoch, backend override or research claim |
