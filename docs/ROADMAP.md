...
> **Document status: ACTIVE.** [GitHub Project #4](https://github.com/users/daffieeee-arch/projects/4) is the central delivery roadmap. [`PROJECT_V2_REBASE.md`](PROJECT_V2_REBASE.md) and [`../roadmap/project-v2-content-migration-ledger.json`](../roadmap/project-v2-content-migration-ledger.json) preserve the completed G0 migration contract and evidence. B4/[#83](https://github.com/daffieeee-arch/Solana-Quant-Bot/issues/83) is the concrete current delivery. PR #94 remains `Fixture` evidence. A separately approved [metadata-only run](research/OF1_STAGED_ACQUISITION.md#observed-checksum-compatibility-offline-correction) completed; no CAR payload or new run is authorized. The draft `ENGINEERING_VALIDATION_ONLY` lease in [`research/B4_ENGINEERING_VALIDATION_LEASE_PLAN.md`](research/B4_ENGINEERING_VALIDATION_LEASE_PLAN.md) remains `approved: false`, distinct from the expired approval retained outside Git.
...
| PR 4 / B4 #83 | **In progress / ACTIVE NOW / Unproven:** four authentic metadata publications, no CAR payload. The [staged path](research/OF1_STAGED_ACQUISITION.md) retains the original [fixture report](research/OF1_ACQUISITION_EVIDENCE.md) separately | checksum correction and separate [graphical monitor](research/B4_DOWNLOAD_MONITOR.md) are merged; paced fixture payload, restart, receipts and explicit coverage limits are visible, not authentic payload proof | new [metadata GO](research/OF1_METADATA_RUN_PROPOSAL.md) and separate payload GO required after final binary preparation; no B4 completion, root membership, full epoch, backend override or research claim |
