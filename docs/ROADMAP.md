...
| PR 4 / B4 #83 | **In progress / ACTIVE NOW:** bounded Old Faithful acquisition. PR #94 B4A and the [Rust offline transport integration](research/OF1_OFFLINE_TRANSPORT.md) are `Fixture` partial results only; [visible end-to-end report](research/OF1_TRANSPORT_EVIDENCE.md) | live terminal/TUI progress, verified receipts, deterministic resume and explicit coverage/gaps | no acquisition run is currently authorized; fixture integration does not authorize one or complete B4; Jetstreamer SHA is a candidate pin only; no full epoch, silent backend override or research claim |
