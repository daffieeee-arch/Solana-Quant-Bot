...
| PR 4 / B4 #83 | **In progress / ACTIVE NOW / Unproven:** four authentic metadata publications and one 45,051-byte CAR range. The [read-only verifier](research/OF1_RECORDED_CAR_VERIFICATION.md) checks the captured archival envelope and preserves its original failure | separate capture, Raw/receipt, CAR/slot and domain-status display; corrected 66-node/65-link verification, no Transaction envelopes in this one range | no further acquisition, B4 completion, root membership, domain/Pump decode, full epoch, backend override or research claim |
