...
| PR 4 / B4 #83 | **In progress / ACTIVE NOW:** bounded Old Faithful acquisition. PR #94 landed B4A as a `Fixture` partial result only. Offline remainder: [`research/B4_OFFLINE_REMAINDER.md`](research/B4_OFFLINE_REMAINDER.md) plus `scripts/b4-offline-remainder.mjs` | live terminal/TUI progress, verified receipts, deterministic resume and explicit coverage/gaps | no acquisition run is currently authorized; B4A and the remainder do not authorize one; remainder is not live B4B; Jetstreamer SHA is a candidate pin only; no full epoch, silent backend override or research claim |
