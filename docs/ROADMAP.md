...
| PR 4 / B4 #83 | **In progress / ACTIVE NOW:** bounded Old Faithful acquisition. PR #94 landed B4A as a `Fixture` partial result only. Offline remainder contract: [`research/B4_OFFLINE_REMAINDER.md`](research/B4_OFFLINE_REMAINDER.md) | live terminal/TUI progress, verified receipts, deterministic resume and explicit coverage/gaps | no acquisition run is currently authorized; B4A and the remainder contract do not authorize one; remainder is not live B4B; no full epoch, silent backend override or research claim |
