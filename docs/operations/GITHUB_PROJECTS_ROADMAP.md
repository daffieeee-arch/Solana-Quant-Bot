...
During migration G0 was the single concrete `ACTIVE NOW`. G0 and B2A are now complete: B3/#82 is the concrete `ACTIVE NOW`, B4/#83 alone is `NEXT`, and B5–B8 remain `LATER` until their direct predecessor is accepted. Program/epic routing cannot create a second concrete delivery head.
