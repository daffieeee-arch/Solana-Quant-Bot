...
During migration G0 was the single concrete `ACTIVE NOW`. G0, B2A and B3 are now complete: B3/#82 is `Done` / `SUPERSEDED` / `Fixture`; B4/#83 is the concrete `ACTIVE NOW`; B5/#84 alone is `NEXT`; and B6–B8 remain `LATER`. Program/epic routing cannot create a second concrete delivery head. No B4 acquisition run is authorized.
