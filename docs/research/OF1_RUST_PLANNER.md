...
`rust/of1-range-recorder` owns the new bounded **planning** path. It is not yet a recorder of acquired Raw and cannot construct a transport. Both JavaScript implementations remain untouched pending invariant salvage and replacement. Their passing tests do not establish durable acquisition.
...
The [dependency review](../../rust/of1-range-recorder/dependency-review.json) lists all locked packages, license expressions and build-script presence. Dependencies are serde/JSON, hashes, typed errors and tempfile for tests. `fs2` is deferred until PR2 needs the writer lock. Existing lockfiles remain unchanged. No native installation, Solana SDK, Jetstreamer runtime, database or HTTP dependency is added.

Build-script roles are compiler/platform cfg and capability detection for generic-array, proc-macro2, quote, serde, serde_core, serde_json, thiserror, zmij, libc, getrandom and rustix. Their registry checksums are lock-bound. Execution of builds, tests and reports uses the existing syscall network-deny launcher. Future manifest/lock changes fail before CI fetch until reviewed; resolved license/build-script metadata must match the review.
...
Use pinned Node 22.23.2/Rust 1.97.1. `--all` runs fmt, clippy, tests, build and twice-regenerated artifact checks with sockets denied. `--print` prints artifact text for review without overwriting committed evidence. Temporary fixtures are removed afterward. The default npm build and Roadmap Sync remain unchanged.

## Remaining PRs and preservation gate

1. **This PR:** Rust schema/index/range planning and visible fixture report.
2. **Durability:** Raw/receipt mixed generations and overwrite, immutable publication, reservations before dispatch, failed-attempt/restart accounting and persistent deadline. The budget bug predates Cursor in PR94.
3. **Offline integration:** loopback-only range/stream tests, exact progress, no external transport; remove both JS implementations only after invariant migration and parity tests.
4. **Lease/evidence:** separate metadata GO from payload GO, replace incomplete approval checklists and explicitly pin historical PR evidence. Live activity remains unauthorized.

The [salvage inventory](../../schemas/acquisition/of1/test-salvage.json) binds baseline file hashes, test classes and open regressions. No old test, script, B3 matrix or historical ledger is removed or rewritten in this PR. Git is the archive. Rollback is a normal revert; user capture directories are never modified or imported.
