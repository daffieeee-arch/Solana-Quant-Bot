//! Offline metadata/payload parity. Only the provenance-bound SHA regression is
//! observed source text; all other generated bytes remain synthetic fixtures.
...
fn complete(store: &mut AcquisitionStore<TestClock>, sequence: u64) {
    ...
    complete_bytes(store, sequence, &data);
}
fn complete_bytes(store: &mut AcquisitionStore<TestClock>, sequence: u64, data: &[u8]) {
    let request = store.request(sequence).unwrap().clone();
...
#[test]
fn observed_sidecar_plans_same_payload_as_basename_without_source_path_io() {
    // Only the sidecar is authentic. Index/CID/HEAD, clock and authority below
    // are local fixtures; no authentic payload or executable transition occurs.
    let observed = include_bytes!("../../../schemas/acquisition/of1/epoch-978-observed.sha256");
    let digest = std::str::from_utf8(&observed[..64]).unwrap();
    let basename = format!("{digest}  epoch-978.car\n");
    let mut plans = Vec::new();
    for sidecar in [observed.as_slice(), basename.as_bytes()] {
        let h = Harness::new();
        let mut store = h.create();
        complete(&mut store, 0);
        complete_bytes(&mut store, 1, sidecar);
        complete(&mut store, 2);
        complete(&mut store, 3);
        let objects = (0..4)
            .map(|n| store.published(n).unwrap().unwrap())
            .collect::<Vec<_>>();
        let before = fs::read(&objects[1].raw_path).unwrap();
        let plan =
            derive_payload_from_metadata(&h.plan, &objects, 422_496_000, 422_496_001).unwrap();
        assert_eq!(
            serde_json::to_value(&plan).unwrap()["declared_car_sha256"],
            digest
        );
        ...
        assert_eq!(
            ...
            RequestKind::CarRange {
                ...
                end_exclusive: 96,
                total: 1024,
                strong_etag: Some("\"fixture\"".into()),
            }
        );
        assert_eq!(objects[1].receipt.source_path, "/978/epoch-978.sha256");
        assert_eq!(fs::read(&objects[1].raw_path).unwrap(), before);
        assert!(objects.iter().all(|p| p.raw_path.starts_with(&h.root)));
        assert!(store.published(4).unwrap().is_none());

        // A changed Raw byte must fail the original receipt binding, even
        // though the changed byte is merely in the filename annotation.
        let mut changed = before;
        let last = changed.len() - 2;
        changed[last] ^= 1;
        fs::write(&objects[1].raw_path, changed).unwrap();
        assert!(matches!(
            derive_payload_from_metadata(&h.plan, &objects, 422_496_000, 422_496_001),
            Err(StoreError::Corrupt)
        ));
        plans.push(plan);
    }
    assert_eq!(plans[0].requests(), plans[1].requests());
    assert_eq!(
        serde_json::to_value(&plans[0]).unwrap()["source_fingerprint"],
        serde_json::to_value(&plans[1]).unwrap()["source_fingerprint"]
    );
    // Different source bytes/receipts must NOT be normalized into one identity.
    assert_ne!(
        plans[0].metadata_receipt_sha256(),
        plans[1].metadata_receipt_sha256()
    );
}
