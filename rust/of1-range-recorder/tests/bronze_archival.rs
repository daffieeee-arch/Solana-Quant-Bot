//! Offline archival-envelope preparation, not a Solana/Pump wire decoder.
use of1_range_recorder::car::{
    ArchivalKind, ByteSpan, CarError, VerificationLimits, inspect_slot_sections,
...
fn sealed_vector_retains_one_atomic_opaque_transaction_and_original_verifier_report() {
    ...
    let continuation = report.archival_nodes[0].dataframe.as_ref().unwrap();
...
fn raw_u64_and_negative_cbor_boundaries_are_decimal_strings_not_float_or_zero() {
    ...
    .concat();
    let tx = transaction(u64::MAX, &data, Some(uint(u64::MAX)));
    ...
    assert_eq!(
        tx.transaction_position.as_deref(),
...
fn repeated_entry_reference_is_ambiguous_even_when_integrity_closure_is_valid() {
    let tx = transaction(SLOT, &frame(b"A"), Some(uint(17)));
    let entry = [
        vec![0x84, 1, 0, 0x58, 32],
        vec![0xaa; 32],
        vec![0x81],
...
#[test]
fn explicit_position_is_preserved_without_replacing_archival_order() {
    let data = [vec![0x85, 6], uint(u64::MAX), vec![0xf6, 0xf6, 0x40]].concat();
    let tx = transaction(SLOT, &data, Some(uint(17)));
    let bytes = graph(SLOT, std::slice::from_ref(&tx), &[cid(&tx)]);
    let report = inspect_slot_sections(SLOT, &bytes, limits()).unwrap();
    let tx = &report.transaction_envelopes[0];
    assert_eq!(tx.transaction_position.as_deref(), Some("17"));
    assert_eq!(tx.archival_ordinal, 0);
    assert_eq!(tx.data.checksum.as_deref(), Some("18446744073709551615"));
}

#[test]
fn missing_or_unsupported_dataframes_and_schema_extensions_fail_closed() {
    for invalid in [
        vec![0xf6],
        vec![0x85, 7, 0xf6, 0xf6, 0xf6, 0x40],
        vec![0x84, 6, 0xf6, 0xf6, 0x40],
    ] {
        let tx = transaction(SLOT, &invalid, None);
        let bytes = graph(SLOT, std::slice::from_ref(&tx), &[cid(&tx)]);
        assert_eq!(
            inspect_slot_sections(SLOT, &bytes, limits()),
            Err(CarError::Schema)
    ...
    assert_eq!(
        inspect_slot_sections(SLOT, &section(&[0x81, 99]), limits()),
        Err(CarError::Schema)
    );
    assert_eq!(
        inspect_slot_sections(SLOT, &section(&[0x81, 3]), limits()),
        Err(CarError::UnsupportedNode)
    );
    let missing_metadata = [vec![0x84, 0], frame(b"A"), vec![0xf6], uint(SLOT)].concat();
    let bytes = graph(
        SLOT,
        std::slice::from_ref(&missing_metadata),
        &[cid(&missing_metadata)],
    );
    assert_eq!(
        inspect_slot_sections(SLOT, &bytes, limits()),
        Err(CarError::Schema)
...
#[test]
fn no_partial_facts_escape_cid_truncation_slot_missing_link_or_drift_errors() {
    let bytes = sealed();
    for end in 0..bytes.len() {
        assert!(inspect_slot_sections(SLOT, &bytes[..end], limits()).is_err());
    }
    for index in 0..bytes.len() {
        let mut changed = bytes.clone();
        changed[index] ^= 1;
        assert!(inspect_slot_sections(SLOT, &changed, limits()).is_err());
    }
    assert_eq!(
        inspect_slot_sections(SLOT + 1, &bytes, limits()),
        Err(CarError::Slot)
    );
    let first_len = inspect_slot_sections(SLOT, &bytes, limits())
        .unwrap()
        .archival_nodes[0]
        .section_span
        .length;
    assert_eq!(
        inspect_slot_sections(SLOT, &bytes[first_len..], limits()),
        Err(CarError::MissingLink)
    );
}

#[test]
fn exact_output_and_spans_are_deterministic_across_chunk_assembly() {
    let bytes = sealed();
    let expected =
        serde_json::to_vec(&inspect_slot_sections(SLOT, &bytes, limits()).unwrap()).unwrap();
    for size in 1..=bytes.len() {
        let assembled: Vec<_> = bytes.chunks(size).flatten().copied().collect();
        assert_eq!(
            serde_json::to_vec(&inspect_slot_sections(SLOT, &assembled, limits()).unwrap())
                .unwrap(),
            expected
        );
    }
    let report = inspect_slot_sections(SLOT, &bytes, limits()).unwrap();
    assert_eq!(
        report.archival_nodes.last().unwrap().kind,
        ArchivalKind::Block
    );
    assert_eq!(
        report
            .archival_nodes
            .iter()
            .map(|node| node.section_span.length)
            .sum::<usize>(),
        bytes.len()
    );
}
