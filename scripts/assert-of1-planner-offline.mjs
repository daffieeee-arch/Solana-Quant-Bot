#!/usr/bin/env node
// B4 planning-only gate. Reuses the existing syscall-deny launcher, not B3 domain logic.
...
export function validateOf1PlannerInputs(manifest, lock, sources) {
  ...
  for (const [path, source] of Object.entries(sources)) {
    if (path === 'build.rs' || /\b(?:TcpStream|TcpListener|UdpSocket|Command)\b|std::net|std::process::Command|\bunsafe\s*\{|#\s*\[\s*path\s*=/u.test(source)) {
...
async function run(mode) {
  ...
  try {
    ...
    const build = isolated('cargo', ['+1.97.1', 'build', ...manifest, '--locked', '--offline', '--bin', 'of1-plan-evidence', '--message-format=json-render-diagnostics']);
    ...
    process.stdout.write('OF1 planner offline graph/fmt/clippy/tests/evidence PASS\n');
  } finally {
