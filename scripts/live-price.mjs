...
const sigs = await rpc('getSignaturesForAddress', [MINT, { limit: 40 }]);
let checked = 0;
for (const s of (sigs ?? [])) {
  ...
  const payload = { transaction: { transaction: tx } }; // json encoding → inner.transaction.message
  const g = parseGenericSwap(payload);
  console.log('sig', s.signature.slice(0, 8), '| kind', g?.kind, '| priceLamports/tok', g?.priceLamportsPerToken, '| SOL÷token ≈', g?.priceLamportsPerToken ? (g.priceLamportsPerToken/1e9).toExponential(2) : 'n/a', 'SOL/tok | wsolΔ', g?.wsolRawDelta, '| tokenΔ', g?.baseRawDelta, '| mint', g?.mint?.slice(0, 8));
  if (++checked >= 4) break;
}
if (!checked) console.log('geen swap-txns');