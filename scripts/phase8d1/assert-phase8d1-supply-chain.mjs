...
`;
  ...
  if(/secrets\.(?!GITHUB_TOKEN)/.test(input.recoveryText)||!containsAll(input.recoveryScript,['evaluatePackageMetadataAttempts','maxAttempts','nextDelaySeconds','PACKAGE_METADATA_TOTAL_TIMEOUT_HOLD','fetchAllPackageVersions','package-version-inventory.json','final-readonly-recheck.json','inspect-artifact-zip.py','evaluateAttestationEvidence','buildExpectedBuilderId','originalPublishRunAttempt','expectedBuilderId','validateOriginalArtifactMetadata','validateOriginalStateChain','unauthenticatedPullDenied','authenticatedPullSucceeded','provenancePresent','spdxSbomPresent','digestRetestPassed','zeroMutationEvidence']))errors.push('missing recovery read-only evidence gates');
  if(!exactKeys(recoveryContract,['schemaVersion','recoveryMode','rootCauseCategory','imageSourceSha','originalPublishRunId','originalPublishRunAttempt','originalFinalVerdict','confirmation','repository','images','originalArtifacts','packageMetadataRetry','buildStack','mutations'])||recoveryContract?.schemaVersion!=='PHASE8D1_EXISTING_DIGEST_RECOVERY_CONTRACT_1'||recoveryContract?.imageSourceSha!=='9ed8d5b8d8b67284c8fc20c164f6816bbfc0c180'||recoveryContract?.originalPublishRunId!==32641496527||recoveryContract?.originalPublishRunAttempt!==1||recoveryContract?.confirmation!=='RECOVER_EXISTING_PHASE8D_IMAGES'||recoveryContract?.images?.length!==2||recoveryContract?.originalArtifacts?.length!==5||recoveryContract?.packageMetadataRetry?.maxAttempts!==6||recoveryContract?.packageMetadataRetry?.maxDelaySeconds!==15||recoveryContract?.packageMetadataRetry?.maxTotalWaitSeconds!==90||Object.values(recoveryContract?.mutations??{}).some(value=>value!==false))errors.push('invalid recovery contract');
