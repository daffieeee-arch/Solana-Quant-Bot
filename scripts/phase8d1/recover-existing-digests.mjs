...
import { evaluateAttestationEvidence, evaluateAttestationManifestBinding, verifyAttestationLayerBytes } from './recovery-oci.mjs';
  ...
  try{
    ...
    try{
      ...
      for(const item of contract.images){
        ...
        if(metadata?.name!==item.package||metadata?.package_type!=='container'||metadata?.visibility!=='private')throw new Error(`PACKAGE_VISIBILITY_HOLD:${item.name}`);
        ...
        const target=images.find(image=>image.name===item.name);if(!target.repositoryAccessVerified)throw new Error(`PACKAGE_REPOSITORY_ACCESS_HOLD:${item.name}`);
        finalReadonlyChecks.push({name:item.name,package:item.package,packageHttpStatus:metadataResponse.status,packageType:metadata.package_type,visibility:metadata.visibility,repositoryFullName:metadata.repository?.full_name??null,repositoryAccessVerified:target.repositoryAccessVerified,versionPages:paged.pages,versionCount:canonical.length,tag:item.tag,expectedDigest:item.digest,resolvedDigest:head.digest,unauthenticatedManifestStatus:unauthHead.status,unauthenticatedPullDenied});
      }
