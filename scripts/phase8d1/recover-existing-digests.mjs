...
async function main(){
  ...
  try{
    ...
    for(const item of contract.images){const target=images.find(image=>image.name===item.name);if(target.packageType!=='container')throw new Error(`PACKAGE_TYPE_HOLD:${item.name}`);if(target.visibility!=='private')throw new Error(`PACKAGE_VISIBILITY_HOLD:${item.name}`);if(target.repositoryFullName!==contract.repository)throw new Error(`PACKAGE_REPOSITORY_LINK_HOLD:${item.name}`);}
    const authConfigs=[];
    try{
      for(const item of contract.images){const target=images.find(image=>image.name===item.name),registry=await inspectRegistryImage(item,owner,actor,token),pull=dockerPullEvidence(item,actor,token);authConfigs.push(pull.authConfig);Object.assign(target,{resolvedDigest:registry.tagResolvedDigest,unauthenticatedPullDenied:pull.unauthenticatedPullDenied,authenticatedPullSucceeded:pull.authenticatedPullSucceeded,platform:registry.platform,sourceGitSha:registry.sourceGitSha,provenancePresent:registry.provenancePresent&&registry.provenanceSourceShaPresent,spdxSbomPresent:registry.spdxSbomPresent,imageConfig:{digest:registry.imageConfigDigest,user:registry.runtimeUser,linuxManifestDigest:registry.linuxManifestDigest}});writeJson(join(evidenceDir,`${item.name}-registry-evidence.json`),{schemaVersion:'PHASE8D1_RECOVERY_REGISTRY_EVIDENCE_1',name:item.name,tag:item.tag,expectedDigest:item.digest,resolvedDigest:registry.tagResolvedDigest,platform:registry.platform,imageConfigDigest:registry.imageConfigDigest,runtimeUser:registry.runtimeUser,sourceGitSha:registry.sourceGitSha,provenancePresent:target.provenancePresent,spdxSbomPresent:target.spdxSbomPresent,unauthenticatedPullDenied:pull.unauthenticatedPullDenied,authenticatedPullSucceeded:pull.authenticatedPullSucceeded});}
      if(images.some(image=>image.resolvedDigest!==image.expectedDigest||!image.unauthenticatedPullDenied||!image.authenticatedPullSucceeded||!image.provenancePresent||!image.spdxSbomPresent))throw new Error('REGISTRY_EVIDENCE_HOLD');
      ...
      for(const item of contract.images){
        const metadataResponse=await packageGet(owner,item.package,token,'',finalDeadlineMs),metadata=metadataResponse.body;if(metadataResponse.status!==200)throw new Error(`${metadataResponse.status===401||metadataResponse.status===403?'PACKAGE_API_PERMISSION_HOLD':'PACKAGE_METADATA_HTTP_HOLD'}:${item.name}`);
        if(metadata?.name!==item.package||metadata?.package_type!=='container'||metadata?.visibility!=='private')throw new Error(`PACKAGE_VISIBILITY_HOLD:${item.name}`);if(metadata?.repository?.full_name!==contract.repository)throw new Error(`PACKAGE_REPOSITORY_LINK_HOLD:${item.name}`);
        const paged=await fetchAllPackageVersions({initialUrl:`${API}/users/${owner}/packages/container/${item.package}/versions?per_page=100`,token,deadlineMs:finalDeadlineMs}),canonical=canonicalVersions(paged.versions),tag=item.tag.slice(item.tag.lastIndexOf(':')+1),matching=canonical.filter(version=>version.tags.includes(tag));afterVersions[item.name]=canonical;afterPages[item.name]=paged.pages;if(matching.length!==1)throw new Error(`PACKAGE_TAG_METADATA_HOLD:${item.name}`);
        const bearer=await registryToken(owner,item.package,actor,token),head=await registryFetch(owner,item.package,'manifests',tag,bearer,{method:'HEAD'}),unauthHead=await registryFetch(owner,item.package,'manifests',item.digest,null,{method:'HEAD'}),unauthenticatedPullDenied=unauthenticatedDockerPullDenied(item);
        if(head.status!==200||head.digest!==item.digest)throw new Error(`FINAL_TAG_DIGEST_DRIFT:${item.name}`);if(![401,403,404].includes(unauthHead.status)||!unauthenticatedPullDenied)throw new Error(`FINAL_PACKAGE_PRIVACY_HOLD:${item.name}`);
        finalReadonlyChecks.push({name:item.name,package:item.package,packageHttpStatus:metadataResponse.status,packageType:metadata.package_type,visibility:metadata.visibility,repositoryFullName:metadata.repository.full_name,versionPages:paged.pages,versionCount:canonical.length,tag:item.tag,expectedDigest:item.digest,resolvedDigest:head.digest,unauthenticatedManifestStatus:unauthHead.status,unauthenticatedPullDenied});
      }
      ...
      writeJson(join(evidenceDir,'package-version-inventory.json'),inventory);writeJson(join(evidenceDir,'final-readonly-recheck.json'),{schemaVersion:'PHASE8D1_RECOVERY_FINAL_READONLY_RECHECK_1',packages:finalReadonlyChecks,verdict:'PASS'});
