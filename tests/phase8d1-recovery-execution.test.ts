...
describe('Phase 8D1-R read-only recovery execution',()=>{
  it('ships an executable shell entrypoint and bounded Node orchestrator',()=>{
    ...
    expect(text).toContain('evaluateAttestationManifestBinding');expect(text).toContain("layer.mediaType!=='application/vnd.in-toto+json'");expect(text).toContain('verifyAttestationLayerBytes(layer,blob.status,blob.bytes)');expect(text).toContain('attestationSummary');expect(text).not.toMatch(/writeJson\([^\n]+payload/);
